...
Removes phone numbers from message senders and bodies before LLM processing.
"""
...
def sanitize(messages: list[Message]) -> list[Message]:
    """Replace phone numbers in senders and bodies before LLM processing.
    ...
    inside message bodies are replaced with the literal string ``[PHONE]``.

    Args:
        messages: Parsed messages from the chat export.

    Returns:
        New list of Message objects with phone numbers removed.
    """
